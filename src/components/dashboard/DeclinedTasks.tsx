...
const DeclinedTasks: React.FC = () => {
  ...
  const renderTaskColumn = (title: string, tasks: Task[], bgColor: string) => (
    <div
      className={`w-full min-w-[400px] ${bgColor} p-5 bg-opacity-40 rounded-lg overflow-auto max-h-[100%] scrollbar-transparent`}
    >
      <h2 className="text-2xl font-semibold text-gray-700 mb-4">{title}</h2>
      <div className="py-2 overflow-y-auto flex flex-wrap flex-2 gap-2">
        {tasks.map((task) => (
          <TaskCard
            key={task.id}
            task={task}
